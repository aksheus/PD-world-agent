class RobotAgent:
	...
	def __init__(self,pickupCells,dropoffCells):
		...
		self.state=[1,5,0,5,5,5,0,0,0] # since robot always starts at (1,5) and does not carry anything initially
		# this state attribute reperesents current state and must be updated accordingly
		self.adj_move=[(1,0),(0,1),(-1,0),(0,-1)] # this is just to calculate robots neighbouring coordinates
		self.robo_scope=[]
	
	def possibleMoves(self):
		""" sees what possible squares the robo can move to legally
		      and updates robo_scope variable"""
		for x,y in self.adj_move:
			temp=(self.state[0]+x,self.state[1]+y)
	...
	def doPickup(self,pickup_cell): #provide the pickup cell returns true if we did pick up !!
		if (self.state[0],self.state[1])==pickup_cell: #check wether the agent is on the pickup cell
			if self.state[2]==0 and pickup_cell in self.pickupCells: # is the agent is not carrying a star, is the pickup cell indeed a pickupcell
	...
	def doDropoff(self,dropoff_cell): # provide pick up cell as tuple returns true if drop off is successful
		if (self.state[0],self.state[1])==dropoff_cell: # is the robo on the drop off cell
			if self.state[2]==1 and dropoff_cell in self.dropoffCells: # is the robo carrying a star , is the cell indeed a dropoff cell
